...
def get_label(model, model_input, device):
    ...
    pred = []
    all_losses = torch.zeros((batch_size, NUM_CLASSES), device=device)  # logits placeholder

    for i in range(batch_size):
        sample = model_input[i:i+1]  # shape: [1, C, H, W]
        sample_losses = []
        ...
        sample_losses = torch.tensor(sample_losses, device=device)  # shape: [NUM_CLASSES]
        all_losses[i] = sample_losses
        pred_class = torch.argmin(sample_losses)
        pred.append(pred_class.item())

    return torch.tensor(pred, device=device), all_losses.T  # shape: (batch_size,), (NUM_CLASSES, batch_size)
...
if __name__ == '__main__':
    ...
    classify(model = model, data_loader = dataloader, device = device, csv_test_file = './data/test.csv', csv_output_file_name = 'submission.csv')
